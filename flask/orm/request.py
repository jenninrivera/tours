...
assert gotten_new_user['name'] == 'test', 'could not get posted user, problem either with get or post users'
...
assert gotten_deleted_user.get('error') is not None, "error message not returned for non-existent user"
...
assert gotten_patched_user['name'] == 'patched', 'user patch was not persisted to database'
