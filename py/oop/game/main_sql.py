...
if not results:
    _, player2_id = query_db(insert_player_query, {"uname": player2.name})
else:
    player2_id = results[0]["id"]

winner_id = player1_id if game.winner == player1.name else player2_id
print(winner_id)
_, game_id = query_db(insert_game_query, {"winner": winner_id, "note": ""})

query_db(insert_player_game_query, {"game_id": game_id, "player_id": player1_id})
query_db(insert_player_game_query, {"game_id": game_id, "player_id": player2_id})
